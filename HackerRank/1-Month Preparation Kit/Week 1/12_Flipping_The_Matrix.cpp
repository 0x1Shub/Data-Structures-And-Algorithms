...
int flippingMatrix(int matrix_rows, int matrix_columns, int **matrix)
{
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
            ...
            int top_right = matrix[i][2 * n - j - 1];
            int bottom_left = matrix[2 * n - i - 1][j];
            int bottom_right = matrix[2 * n - i - 1][2 * n - j - 1];
            max_sum += fmax(fmax(top_left, top_right), fmax(bottom_left, bottom_right));
        }
    }
